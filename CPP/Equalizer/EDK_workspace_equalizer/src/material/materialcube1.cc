...
MaterialCube1::MaterialCube1()
{
}

bool MaterialCube1::load_shaders_from_filepaths(const char * vertex_shader_path, const char * fragment_shader_path){
...
bool MaterialCube1::load_shaders(EDK3::ref_ptr<EDK3::dev::Shader> vertex_shader,
                                 EDK3::ref_ptr<EDK3::dev::Shader> fragment_shader){
...
bool MaterialCube1::enable(const EDK3::MaterialSettings* mat_setting) const{
  SettingsMatCube1* matsettings = (SettingsMatCube1*)mat_setting;
...
void MaterialCube1::setupCamera(const float projecton[16], const float view[16]) const{
...
void MaterialCube1::setupModel(const float m[16]) const{

  int model_id = get_program()->get_uniform_position("u_m_matrix");
  get_program()->set_uniform_value(model_id, EDK3::T_MAT_4x4, m);
}

unsigned int MaterialCube1::num_attributes_required() const{
  return 2;
}

EDK3::Attribute MaterialCube1::attribute_at_index(const unsigned int attrib_idx) const{
  switch (attrib_idx) {
  case 0:
...
EDK3::Type MaterialCube1::attribute_type_at_index(const unsigned int attrib_index) const{
...
void MaterialCube1::set_program(EDK3::ref_ptr<EDK3::dev::Program> program){
  program_ = program;
}

EDK3::dev::Program * MaterialCube1::get_program()
{
  return program_.get();
}

const EDK3::dev::Program * MaterialCube1::get_program() const
{
  return program_.get();
}
