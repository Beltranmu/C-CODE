...
s16 VECTOR_destroy(Vector* vector){
  ...
  for (int i = vector->tail_-1; i>=vector->head_ ; --i) {
    return_code = vector->storage_->ops_->reset(vector->storage_ + i);
  }
...
s16 VECTOR_softReset(Vector* vector) {
  ...
  s16 return_code = kErrorCode_Ok;
  for (int i = vector->tail_ - 1; i >= vector->head_ ; --i) {
    return_code = vector->storage_->ops_->softReset(vector->storage_ + i);
  }
  vector->tail_ = 0;
  return return_code;
}
...
s16 VECTOR_reset(Vector* vector) {
  ...
  for (int i = vector->head_; i < vector->tail_ ; ++i) {
    return_code = (vector->storage_ + i)->ops_->reset(vector->storage_ + i);
  }
  vector->tail_ = 0;
...
s16 VECTOR_resize(Vector* vector, u16 new_size) {
  ...
  if (new_size == vector->capacity_) {
    ...
    for (int i = 0; i < vector->tail_ ; ++i) {
      return_code = aux_storage->ops_->setData(aux_storage + i, (vector->storage_ + i)->data_,
    ...
    for (int i = 0; i < new_size ; ++i) {
      ...
      if (return_code == kErrorCode_Ok) {
        return_code = aux_storage->ops_->setData(aux_storage + i,
          (vector->storage_ + i)->data_,
          (vector->storage_ + i)->size_);
      }
    }

    for (int i = vector->capacity_ - 1 ; i >= new_size ; --i) {
      return_code = vector->storage_->ops_->reset(vector->storage_ + i);
    }
...
Vector* VECTOR_create(u16 capacity) {
  ...
  Vector* new_vector = MM->malloc(sizeof(Vector));
  if (NULL == new_vector) {
    return NULL;
  }
  new_vector->head_ = 0;
