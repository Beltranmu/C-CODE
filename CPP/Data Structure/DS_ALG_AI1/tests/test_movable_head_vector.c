...
int main() {
  ...
  MovableHeadVector *v = NULL;
	v = MOVABLE_HEAD_VECTOR_create(1);
	if (NULL == v) {
		printf("\n create returned a null node in vector for ops");
		return -1;
	}
	// vectors to work with
  MovableHeadVector *vector_1 = NULL;
  MovableHeadVector *vector_2 = NULL;
  MovableHeadVector *vector_3 = NULL;
	vector_1 = MOVABLE_HEAD_VECTOR_create(kCapacityVector1);
	if (NULL == vector_1) {
		printf("\n create returned a null node in vector_1\n");
		return -1;
	}
	vector_2 = MOVABLE_HEAD_VECTOR_create(kCapacityVector2);
	if (NULL == vector_2) {
		printf("\n create returned a null node in vector_2\n");
		return -1;
	}
	vector_3 = MOVABLE_HEAD_VECTOR_create(kCapacityVector3);
	if (NULL == vector_3) {
	...
	printf("\n\n---------------- NULL BATTERY ----------------\n\n");
	printf("\n\n# Test Create\n");
	vector_2 = MOVABLE_HEAD_VECTOR_create(0);
	if (NULL != vector_2) {
