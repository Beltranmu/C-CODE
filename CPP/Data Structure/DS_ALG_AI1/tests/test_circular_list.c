...
int main() {
	...
	Cricular_List *ls = NULL;
	ls = CIRCULAR_LIST_create(1);
	if (NULL == ls) {
		printf("\n create returned a null node in list for ops");
		return -1;
	}
	// vectors to work with
	Cricular_List *list_1 = NULL;
	Cricular_List *list_2 = NULL;
	Cricular_List *list_3 = NULL;
	list_1 = CIRCULAR_LIST_create(kCapacityList1);
	if (NULL == list_1) {
		printf("\n create returned a null node in list_1\n");
		return -1;
	}
	list_2 = CIRCULAR_LIST_create(kCapacityList2);
	if (NULL == list_2) {
		printf("\n create returned a null node in list_2\n");
		return -1;
	}
	list_3 = CIRCULAR_LIST_create(kCapacityList3);
	if (NULL == list_3) {
